...
def build_detection_semisup_train_loader_two_crops(cfg, mapper=None):
    if cfg.DATASETS.CROSS_DATASET:  # cross-dataset (e.g., coco-additional)  ZY: meet
    ...
    label_dataset = DatasetFromList(label_dicts, copy=False)    # ZY: dt2内部函数, label_dicts -> label_dataset 目前不太懂,先当固定用法
    ...
    if mapper is None:
        mapper = DatasetMapper(cfg, True)
    label_dataset = MapDataset(label_dataset, mapper)    # ZY: dt2内部函数, 目前不太懂,先当固定用法
    unlabel_dataset = MapDataset(unlabel_dataset, mapper)

    sampler_name = cfg.DATALOADER.SAMPLER_TRAIN    # ZY: dt2内部默认
    logger = logging.getLogger(__name__)
    logger.info("Using training sampler {}".format(sampler_name))
    if sampler_name == "TrainingSampler":
        label_sampler = TrainingSampler(len(label_dataset))    # ZY: dt2内部函数, 目前不太懂,先当固定用法. 用于 DataLoader中的 sampler参数
...
def build_semisup_batch_data_loader_two_crop(
    ...
    *,
    ...
    world_size = get_world_size()    # ZY: dt2内部函数, Returns the number of processes in the current process group
    assert (
    ...
    if aspect_ratio_grouping:
        ...
        )    # ZY: 作者自己的代码, Batch data that have similar aspect ratio together
    else:
